...
export default {
    ...
    methods: {
        ...
        // 子组件传值
        isEmit(data) {
            let { add, count, id, item, del, new_item } = data;
            this.handleEmit(add, count, id, item, del, new_item)
            this.add = data.add || false;
            if (data.count) this.count = data.count;
            if (data.id) this.item_id = data.id;
            if (data.item) {
                this.addToCart(data.item, data);
            }
            if (data.del) {
                data.del == -1
        ...
        // 缓存已获取过的物品
        cacheItemData(item) {
            if (!this.cache_data[this.server]) this.cache_data[this.server] = {};
            if (!this.cache_data[this.server][item.ID]) {
                item.count = 1;
                this.cache_data[this.server][item.ID] = item;
            }
        },
    },
    created() {
